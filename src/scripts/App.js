/** Tracks which `data-module` values already ran on a given element (Swup + full-doc init both scan #main-content). */
const MODULE_INIT = Symbol.for('app.domModule.initialized');
...
export default class App {
  ...
  initModule(Constructor, element) {
    ...
    let inited = element[MODULE_INIT];
    if (!inited) {
      inited = new Set();
      element[MODULE_INIT] = inited;
    }
    if (inited.has(name)) {
      return;
    }
